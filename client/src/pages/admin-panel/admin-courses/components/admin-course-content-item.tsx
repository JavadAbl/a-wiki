...
const AdminCourseContentItem = ({
  ...
  const [inputValue, setInputValue] = useState<string | number>(content.order);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setInputValue(val);
    setIsEditing(Number(val) !== content.order);
  };

  const handleReject = () => {
    setInputValue(content.order);
    setIsEditing(false);
  };

  const handleAccept = async () => {
    setIsLoading(true);
    await onAcceptOrder(content.id, Number(inputValue));
    setIsEditing(false);
    setIsLoading(false);
  };
  ...
  return (
    <div className="flex items-center justify-between gap-2 px-3 py-1.5 bg-muted rounded-full text-sm font-medium">
      <div className={cn("flex items-center gap-[16px]")}>
        <span className="truncate max-w-[100px] flex-1">{content.title}</span>
