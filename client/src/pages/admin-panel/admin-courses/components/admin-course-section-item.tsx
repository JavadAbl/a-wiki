...
import { Check, X, Pencil } from "lucide-react"; // Added Pencil icon
import { cn } from "#lib/utils";
import type { SectionDto } from "../../../../features/course/dto/section.dto";
import { useSectionUpdateMutation } from "../../../../features/course/course-api";
...
const AdminCourseSectionItem = ({
  ...
  const [orderValue, setOrderValue] = useState<string | number>(section.order);
  const [isEditingOrder, setIsEditingOrder] = useState(false);

  // Separate state for Title editing
  const [titleValue, setTitleValue] = useState<string>(section.title);
  const [isEditingTitle, setIsEditingTitle] = useState(false);

  const [isLoading, setIsLoading] = useState(false);

  const [mutateUpdateSection] = useSectionUpdateMutation();

  // --- Order Handlers ---
  const handleOrderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = e.target.value;
    setOrderValue(val);
    setIsEditingOrder(Number(val) !== section.order);
  };

  const handleRejectOrder = () => {
    setOrderValue(section.order);
  ...
  const handleAcceptOrder = async () => {
    setIsLoading(true);
    try {
      await onAcceptOrder(section.id, Number(orderValue));
      setIsEditingOrder(false);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleUpdateTitle = async () => {
    setIsLoading(true);
    try {
      await mutateUpdateSection({
        sectionId: section.id,
        body: { title: titleValue },
      }).unwrap(); // .unwrap() is recommended for RTK Query to catch errors properly
      setIsEditingTitle(false);
    } catch (error) {
      console.error("Failed to update title", error);
      // Optional: revert to original title on failure
      ...
      >
        <span className="flex items-center gap-1 truncate flex-1">
          ...
          {isEditingTitle ? (
            ...
            <span className="rounded-md px-1 truncate">{section.title}</span>
          )}
        </span>
