...
import { usePartCreateMutation } from "../../../../features/course/course-api";
import {
  PartCreateSchema,
  type PartCreateDto,
} from "../../../../features/course/schemas/part-create-schema";
...
export default function PartCreate({ isOpen, setIsOpen, sectionId }: Props) {
  const form = useForm<PartCreateDto>({
  ...
  async function handleSubmit(data: PartCreateDto) {
    ...
    if (!res.error) {
      toast.success("قسمت با موفقیت ایجاد شد");
      setIsOpen(false);
      form.reset();
    }
  }
  ...
  return (
    <Modal open={isOpen} onOpenChange={setIsOpen} title="ایجاد قسمت جدید">
      <form
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel htmlFor="title">عنوان قسمت</FieldLabel>
              <FormInput
                {...field}
                id="title"
                aria-invalid={fieldState.invalid}
                placeholder="عنوان قسمت را وارد کنید"
                autoComplete="off"
              />
